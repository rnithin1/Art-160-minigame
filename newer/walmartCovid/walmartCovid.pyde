...
class Game:
    ...
    def initializeFoods(self, FOOD_IMAGE, foodCount=5, food_types=food_types):
        ...
        if not self.food_list:
            self.food_list = random.choice([tup for tup in partition(self.gameSettings['winFoodCount']) if len(tup) == food_types])

        leftover = 0
        if self.food_list:
            food_gen = random.choice([tup for tup in partition(foodCount) if len(tup) == food_types])
            for i in range(1, len(self.food_list) + 1):
                for j in range(food_gen[i - 1]):
                    ...
                    while self.Board.mapValue(foodLocation) in self.Board.unwalkable:
                        foodLocation = random.randint(0, self.Board.tileCount - 1)
                    self.Board.tiles[foodLocation].updateImage(loadImage(IMAGE_DIRECTORY_NAME + FOOD_DIRECTORY_NAME + str(i) + ".png"))
